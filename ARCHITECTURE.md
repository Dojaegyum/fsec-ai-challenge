...
- ✅ **2026-09-03 부터 실제로 지워집니다.** `GET /api/cron/purge` 가 붙고
  `src/vercel.json` 의 `crons` 가 하루 1회(UTC 18:00 = KST 03:00) 깨웁니다
  → [08-14-api.md](spec/common/08-14-api.md) §6.3.
  <br>⛔ **그전까지는 아무것도 안 지워졌습니다.** 모듈도 조립도 서 있었는데
  **부르는 라우트가 없었고**, 관계형 DB 쪽 포트(`purgeCaseStore`)가 미설정
  대역이라 불렀어도 그 자리에서 터졌습니다 — **「180일 뒤 지웁니다」가 한 번도
  지켜진 적이 없었다는 뜻입니다.** 절차가 문서에 있다는 것과 그것이 돈다는 것은
  다릅니다
...
- ~~NER 모델·서비스 선택~~ → **모델은 gemma3:4b, 서비스는 RunPod Pod** (2026-08-26 확정).
  ...
  **걸어서 확인**했습니다 — 「김민수」가 `[이름-1]` 로 바뀌고, 서비스를 끄면
  ...
  ⬜ **배포본은 아직 안 붙였습니다.** 켜기 전 조건이던 토큰화 제외 목록 배선은
