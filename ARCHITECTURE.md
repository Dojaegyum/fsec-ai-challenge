...
| 읽기 맡김 | `transcriber` | 모델 서비스에 STT·OCR을 맡기고 결과를 되묻는다. 화자 판정(먼저 말한 쪽이 A, 말풍선 좌우)은 여기서 한다 — **다만 STT 엔진은 아직 화자를 안 준다**(`faster_whisper_stt.py` 끝의 ⬜ · `no_speakers`), 그래서 오디오는 화자 없이 시간 순서 줄이고 A/B 는 이미지에서만 붙는다 |
