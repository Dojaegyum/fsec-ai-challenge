...
- **결론이 난 제안은 `rfc/`에 남기지 않습니다.** `decisions/`로 옮기고 rfc 파일은 지웁니다 — 같은 주제가 두 곳에 있으면 정본이 헷갈리고 ADR과 역할이 겹칩니다. 논의 과정은 git 히스토리에 남습니다. **`rfc/`에 파일이 있다 = 아직 안 정해졌다.**
- 반대로 **ADR은 지우지 않습니다.** 채택된 것을 뒤집을 때는 새 ADR을 쓰고 기존 ADR을 `대체됨`으로 표시합니다.
