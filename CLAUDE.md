...
| 화면 목업(옛 그림) | `assets/artifacts/plans/08-17-screen-mockups.html` | **동결됨** — 화면 01(S-04)까지만 갱신. 레이아웃은 더 이상 정본이 아니고 **설계 노트만 유효**합니다 ([ADR-029](decisions/029-design-handoff.md)) |
...
- **단 화면 디자인은 예외입니다.** `08-17-screen-mockups.html` 은 동결됐고, 그림의 정본은 **Claude Design 핸드오프**입니다 ([RFC-003](rfc/003-design-handoff.md) · [ADR-029](decisions/029-design-handoff.md)). 시안과 `spec/` 이 어긋나면 어느 쪽도 자동으로 이기지 않습니다 — **사람이 정합니다.**
