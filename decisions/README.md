...
| [068](068-no-admin-screen.md) | 관리자 조회 화면을 만들지 않는다 — 계약 §5 는 폐기하고, 문지기와 기록은 남긴다 | 부분 대체됨(→081) |
...
| [088](088-kb-review-screen.md) | KB 검수 큐 화면을 만든다 — 관리자 계정 하나와 문지기를 살리고, 사건 조회 화면은 계속 두지 않는다 | 채택 |
| [089](089-kb-selector.md) | 발화를 보고 두 묶음 밖의 자료(다른 절차 · 기관 연락처 · 법령 조문)를 고르는 `kb-selector` 를 층 2에 둔다 — 답은 쓰지 않고 id 만 고르는 2단계 선별 · 선별 전용 빠른 모델 · 법령 조문 원문은 검수 없이 후보 | 채택 |
| [090](090-stale-pending-rekick.md) | 없는 증거로 완료 통지하면 404 로 끊고(500 이던 결함), 오래 멈춘 `pending` 은 조회가 완료 처리를 다시 태워 판독을 깨운다 — 90초 임계 · 멱등 · `read` 에 `createdAt` 추가 | 채택 |
| [091](091-evidence-retrying-not-failed.md) | 전사·판독·이름 탐지 서버에 **못 닿은 것**은 `failed` 가 아니라 `processing` 인 채 「재시도중」 — 어댑터가 일시적/최종적을 가르고, 브라우저 폴링이 물을 때마다 다시 맡기며(5초 간격 · 상한 없음), 이름 탐지만 죽었으면 원문을 버리고 다음 폴링에 재토큰화(팟 30분 보관) · 서버 혼자 할 수 있는 다시 맡기기는 `POST /api/cron/evidence-resubmit` | 채택 |
| [092](092-pod-self-heal-and-watcher.md) | 추론 팟은 **세 겹**으로 산다 — 돈은 RunPod auto-pay(사용자가 켬), 프로세스는 팟 안 `watchdog.sh`(20초 · 조치 뒤 유예 180초), 팟은 상시 서버(OCI)의 systemd 감시자(30초 · API 가 없다면 새 팟 · RUNNING 인데 죽었으면 ssh 재시작 → 안 되면 새 팟 · `vercel-env` 로 주소 교체 · Brevo 메일). **연속 실패 횟수를 세지 않는다** · GitHub 스케줄·앱→감시자 신호는 사용자가 물리침 | 채택 |
