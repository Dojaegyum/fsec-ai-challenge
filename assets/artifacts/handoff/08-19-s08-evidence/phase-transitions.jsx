...
function EvidenceContent({ T, at }) { // 증거함(1d) — 레일 + 전사, 폭 chatW 기준
  const rail = [
    ['0812_수신전화.m4a', '통화 녹음 · 전사 완료', C.pii, false, true],
    ['0813_재통화.m4a', '가리는 중 74%', C.pii, true, false],
    ['지급정지_접수문자.png', '캡처 · ◆ 단계 증빙', C.pii, false, false],
    ['신분증_사진.jpg', '제외 — 주민번호 못 가림', C.amber, false, false],
    ['이체내역_0812.png', '대기 중', C.icon, false, false],
  ];
  return (
    <div style={{ position: 'absolute', inset: 0, display: 'grid', gridTemplateColumns: '210px 1fr', fontFamily: F }}>
      <div style={{ padding: '16px 12px', borderRight: `1px solid ${C.hair}`, background: 'oklch(1 0 0 / 1.5%)', ...MOTION.enter(T, at) }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', padding: '0 6px' }}>
          <span style={{ fontSize: 11.5, letterSpacing: '.12em', color: C.icon }}>자료 5</span>
          <span style={{ fontSize: 12, color: C.pii }}>＋ 올리기</span>
        </div>
        <div style={{ display: 'grid', gap: 4, marginTop: 10 }}>
          {rail.map(([name, sub, col, pulse, sel], i) => (
            <div key={name} style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '9px 9px', borderRadius: 10, background: sel ? 'oklch(0.697 0.16 258.2 / 10%)' : 'transparent', border: sel ? '1px solid oklch(0.697 0.16 258.2 / 34%)' : '1px solid transparent', ...MOTION.enter(T, at + 0.1 + i * 0.08) }}>
              <span style={{ width: 7, height: 7, flex: 'none', borderRadius: 999, background: col, animation: pulse ? 'pulse-dot 1.6s ease-in-out infinite' : 'none' }} />
              <div style={{ minWidth: 0, flex: 1 }}>
                <div style={{ fontSize: 12.5, fontWeight: sel ? 600 : 460, color: sel ? C.ink1 : C.ink2, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{name}</div>
                <div style={{ fontSize: 11.5, color: col === C.amber ? C.amber : C.icon }}>{sub}</div>
              </div>
            </div>
          ))}
        </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, padding: '13px 20px', borderBottom: `1px solid ${C.hair}`, ...MOTION.enter(T, at + 0.15) }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 11 }}>
            <span style={{ width: 32, height: 32, display: 'grid', placeItems: 'center', borderRadius: 9, background: 'oklch(0.697 0.16 258.2 / 14%)', border: '1px solid oklch(0.697 0.16 258.2 / 35%)', fontSize: 13, color: C.pii }}>▸</span>
            <div>
              <div style={{ fontSize: 14.5, fontWeight: 640, color: C.ink1 }}>0812_수신전화.m4a</div>
              <div style={{ fontSize: 12, color: C.icon }}>8월 12일 14:22 · 6분 41초</div>
            </div>
          </div>
          <div style={{ display: 'flex', gap: 6 }}><Tok>이름·1</Tok><Tok>계좌·1</Tok></div>
        </div>
        <div style={{ flex: 1, padding: 20, display: 'grid', gap: 15, alignContent: 'start' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '40px 1fr', gap: 12, ...MOTION.enter(T, at + 0.3) }}>
            <span style={{ fontSize: 11.5, color: C.icon, fontFamily: 'ui-monospace,Menlo,monospace', marginTop: 2 }}>00:12</span>
            <div>
              <div style={{ fontSize: 11.5, color: C.icon, marginBottom: 3 }}>상대방</div>
              <p style={{ margin: 0, fontSize: 14, lineHeight: 1.7, color: C.ink2 }}>서울중앙지검 수사관입니다. <Tok>이름·1</Tok> 씨 명의 계좌가 범죄에 연루되어 확인이 필요합니다.</p>
            </div>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '40px 1fr', gap: 12, ...MOTION.enter(T, at + 0.45) }}>
            <span style={{ fontSize: 11.5, color: C.icon, fontFamily: 'ui-monospace,Menlo,monospace', marginTop: 2 }}>02:47</span>
            <div>
              <div style={{ fontSize: 11.5, color: C.icon, marginBottom: 3 }}>상대방 · <span style={{ color: C.amber }}>사칭 정황 구간</span></div>
              <p style={{ margin: 0, fontSize: 14, lineHeight: 1.7, color: C.ink2, borderLeft: '2px solid oklch(0.77 0.117 70.9 / 60%)', paddingLeft: 11 }}>안전계좌로 옮기셔야 합니다. 지금 불러드리는 <Tok>계좌·1</Tok> 로 이체해 주세요.</p>
            </div>
          </div>
        </div>
        <div style={{ padding: '11px 20px', borderTop: `1px solid ${C.hair}`, fontSize: 12, color: C.icon, ...MOTION.enter(T, at + 0.6) }}>
          <span style={{ color: C.pii }}>파란 토큰 = 서버로 안 갔다는 뜻</span> — 서버가 받은 것은 이 화면 그대로입니다
        </div>
      </div>
    </div>
  );
...
function Piece({ tweaks }) {
  ...
  const B = CUES['보드 전환'], P = CUES['패널 등장'], Rt = CUES['챗 복귀'], E = CUES['증거함 전환'];
  ...
  const bVis = clamp(MOTION.move(T, B + 0.6, B + 1.6) - MOTION.move(T, E + 0.3, E + 1.0), 0, 1);   // 보드: 증거함 전환에서 퇴장
  const eVis = clamp(MOTION.move(T, E + 0.5, E + 1.3) - MOTION.move(T, Rt + 0.6, Rt + 1.5), 0, 1);   // 증거함: 챗 복귀에서 퇴장 (빈 화면 없이 교차)
    ...
    <div data-screen-label={`t=${Math.floor(time)}s`} style={{ position: 'absolute', inset: 0, background: C.ground, overflow: 'hidden', fontFamily: F }}>
      ...
      <div style={{ position: 'absolute', left: 0, top: 0, right: 0, height: HEADER, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 22px', borderBottom: `1px solid ${C.hair}`, background: C.stage, zIndex: 5 }}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <span style={{ opacity: eVis }}><Chip>증거함</Chip></span>
          <span style={{ opacity: clamp(bVis + eVis, 0, 1) }}><Chip amber>피해구제 신청까지 D-2</Chip></span>
        </div>
      </div>

      {/* 보드 — 챗이 비켜나면 주인 */}
      <div style={{ position: 'absolute', left: 0, top: HEADER, width: chatW, height: 720 - HEADER, opacity: bVis, transform: `translateY(${(1 - bVis) * 22}px)`, display: bVis < 0.01 ? 'none' : 'block' }}>
        <BoardContent T={T} at={B + 0.7} />
      </div>

      {/* 증거함(1d) — 보드와 같은 자리를 교차로 이어받음 */}
      <div style={{ position: 'absolute', left: 0, top: HEADER, width: chatW, height: 720 - HEADER, opacity: eVis, transform: `translateY(${(1 - eVis) * 22}px)`, display: eVis < 0.01 ? 'none' : 'block' }}>
        <EvidenceContent T={T} at={E + 0.8} />
      </div>
      ...
      <div style={{ position: 'absolute', left: 1280 - PW, top: HEADER + 342, width: PW, height: 720 - HEADER - 342, boxSizing: 'border-box', opacity: miniFade, zIndex: 4, borderLeft: `1px solid ${C.hair}`, background: 'oklch(1 0 0 / 1.5%)', padding: '14px 18px 16px', display: miniFade < 0.01 ? 'none' : 'flex', flexDirection: 'column', fontFamily: F }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, minHeight: 42, borderRadius: 12, border: '1px solid oklch(0.697 0.16 258.2 / 45%)', background: C.surface, boxShadow: '0 0 0 3px oklch(0.697 0.16 258.2 / 10%)', padding: '0 12px' }}>
          <span style={{ fontSize: 12.5, color: 'oklch(0.65 0 89.9)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>무엇이든 물어보세요</span>
      ...
      {tweaks.showCaptions && <Captions style={{ top: 58, bottom: 'auto', zIndex: 20, font: `560 21px ${F}`, color: '#f6f4ef' }} items={[
        ...
        { at: E + 0.5, text: '증거함 — 가려진 것이 문장 안에서 그대로 보입니다' },
