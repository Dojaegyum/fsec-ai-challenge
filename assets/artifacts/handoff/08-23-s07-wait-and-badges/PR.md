...
- **공고 대기 = 턴 2의 2a 「wait-card」** — 1b 아래 풀폭 스트립(47% 진행바)은 카운트다운으로 읽혀 **폐기**. 2a가 대체합니다: 단계 행과 같은 폭, 달력 앵커(시작·지금·만료 예정, D-n·퍼센트 없음, 진행 구간은 horizon 오렌지 oklch(0.811 0.14 66.9) · 기한 앰버(--deadline-urgent)와 다른 토큰), "연락 없는 것이 정상" 카피, 통지 해독 액션 1개, 환급은 날짜 약속 없음. 진행 마커 위치까지 전부 서버 계산값.
- **기한 표시 = 턴 2의 2b 「deadline-badges」** — 별도 DeadlineList 화면은 만들지 않습니다. 배지 3변형만: ① 사용자 기한(앰버 + D-n — D-day는 여기에만) ② 본 기한 지남(중립, 지우지 않음) + 유예(앰버) 병치 — 합치지 않음 ③ 제도 시간(중립, 날짜만) · 환급(배지 없음, "통지가 오면"). 지난 기한 보존은 단계 행이 맡습니다.
