...
| ~~⑫~~ | `spec/common/08-14-features.md` F-02 · `08-14-api.md` §3.3 · `ARCHITECTURE.md` 「읽기 맡김」 | F-02 「STT(화자 분리) … **구현**」 ↔ 오디오 화자 분리는 구현 전(`faster_whisper_stt.py` 끝 ⬜ 「speaker 를 아예 안 채웁니다」 · 배포본 예시 녹음 `shortfalls:["no_speakers"]`) · §3.3 예시의 `speaker: A/B` 는 이미지 좌·우뿐(`transcribe.ts:363`). 2026-09-06 심사 발견 DC-04·E2-P1 — 기능명세서(첨부2)는 #120 으로 먼저 고쳤고 spec 이 뒤따름 | 2026-09-07 손질 — 상태 「부분」+ 구현 전 표시 · §3.3 정정 표에 `speaker` 행 · ARCHITECTURE 한 줄. 화자 분리 자체는 본선 과제 |
