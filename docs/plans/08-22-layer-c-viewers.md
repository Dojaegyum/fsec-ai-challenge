...
> **은퇴(2026-08-26)** — 태스크 9개가 2026-08-23 에 전부 끝났습니다. 세 모듈은 `src/modules/` 에 있고,
> 규칙은 각 모듈의 `README.md` 와 시험이 지킵니다. `doc-filler` 를 뺀 이유는 아래 절에 그대로 남아 있습니다.
...
> **2026-08-23 (저녁) — Task 1 에서 올린 계약 구멍 셋이 전부 닫혔습니다.**
> `steps[]` 의 `step_key`·`after` 는 **싣기로**, `days_left` 는 **아직 안 지난 기한에만**,
> `unconfirmed` 의 어휘는 **「증빙 대기」**로 정해졌습니다 (→ §3.6 · §3.7 · §S-07).
> **서버는 아직 미구현**이라 화면은 계속 픽스처를 먹습니다.
>
...
| 계약 구멍 | 넷이 조용히 있었음 | **전부 닫힘** (2026-08-23) — 서버 구현만 남음 |
...
| 무엇 | 스펙 | 왜 못 하나 |
| --- | --- | --- |
| **히어로 스트립의 D-day 박스** | §S-07 「첫 줄이 답입니다」 | `days_left` 를 **싣기로 확정**됐지만(2026-08-23 · §3.7) **서버가 아직 안 줍니다.** 지금 화면의 목업은 그대로 두고, 값이 오면 `deadline-viewer` 의 `DeadlineBadge` 를 그 자리에 끼웁니다 |
| ~~**사건 진행 레일 (타임라인)**~~ | [§S-07 「사건 진행 레일」](../../spec/frontend/08-14-screens.md) | ✅ **2026-08-27 해결.** 국면을 내리는 칸은 여전히 §3.6 에 없지만 **`body.step_key` 로 유도됩니다** — 칸마다 걸리는 `step_key` 를 정하고 그 단계의 `state` 로 정합니다. 계약도 함께 세웠고 **칸은 넷**입니다(채택 시안 1c) |
