...
| **KB 검수 큐** | 화면 `/admin/kb` 가 코드에 섰습니다(2026-09-06 · [ADR-081](../decisions/081-kb-review-screen.md)). **배포본에서 열리려면 `ADMIN_PASSWORD_HASH` 가 있어야 합니다** — 저장소 시크릿에 `npm run admin:hash` 값을 두고 `vercel-env` 의 `set_admin_password_hash` 로 넣습니다. 넣기 전엔 로그인이 전부 401 이고, 피해자 화면·크론은 그 값과 무관합니다 |
