...
> ⛔⛔ **지금은 `NER_URL` 을 채우지 마세요 — 켜는 것이 끄는 것보다 나쁩니다.**
> 토큰화 제외 목록(`allowedTerms`)이 **전사·챗·증거 세 경로에 안 물려 있습니다**
> (`tokenize()` 가 `?? []` 로 받습니다). 켜면 **문장 안의 기관명이 `[이름-N]` 이 되어**
> (17곳 × 문장 다섯에서 **5~70%** — 말투가 갈랐습니다)
> 경유 서비스를 못 고르고 **에러 없이 슈퍼셋 플랜이 나갑니다.** 지금까지 안 터진
> 것은 `NER_URL` 이 비어서 NER 분기를 건너뛰었기 때문입니다 →
