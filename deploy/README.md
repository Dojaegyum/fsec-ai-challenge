...
| **법령 수집** | 표 셋(마이그레이션 0010)은 공유 DB 에 섰고 첫 수집(법 31조문·시행령 27조문)은 2026-09-06 에 로컬에서 돌렸습니다. 배포본 크론 `/api/cron/kb-collect`(KST 04:00)이 돌려면 `vercel-env` 의 `law_api_oc` 로 `LAW_API_OC` 를 넣어야 합니다 — 검수는 `npm run kb:review` 또는 아래 검수 큐 화면 |
| **KB 검수 큐** | 화면 `/admin/kb` 가 코드에 섰습니다(2026-09-06 · [ADR-088](../decisions/088-kb-review-screen.md)). **배포본에서 열리려면 `ADMIN_PASSWORD_HASH` 가 있어야 합니다** — 저장소 시크릿에 `npm run admin:hash` 값을 두고 `vercel-env` 의 `set_admin_password_hash` 로 넣습니다. 넣기 전엔 로그인이 전부 401 이고, 피해자 화면·크론은 그 값과 무관합니다 |
| **시연 칩** | **켜져 있습니다** — `NEXT_PUBLIC_DEMO_MOCK=1` (2026-09-04). 시작 화면에 「예시 자료로 체험하기」이 주소 없이 보입니다 → 아래 「시연 칩」 |
...
### 시연 칩 — 「예시 자료로 체험하기」 (2026-09-04)

시작 화면(`/start`)의 그 칩은 [합성 자료 셋](../assets/demo/09-01-mock-evidence/)을
한 번에 담는 자리입니다 — 담긴 뒤로는 사람이 고른 파일과 **같은 길**로 올라가고 전사되고
가려집니다(`src/app/start/mock.ts`). 실사용자에게 개발 용어를 보이지 않으려고 2026-09-03 에
숨겼고(감사 D5), 지금은 셋 중 하나면 보입니다.

| 조건 | 언제 |
| --- | --- |
| `NEXT_PUBLIC_DEMO_MOCK=1` 로 빌드 | **대회 기간의 배포본.** `vercel-env` 의 `demo_mock` 에 `1` — 브라우저 번들에 구워지므로 **다시 배포해야** 반영됩니다. 대회가 끝나면 `0` |
| 주소에 `?demo` | 스위치가 꺼진 배포본에서도 — `https://fin-ally-khaki.vercel.app/start?demo` |
| dev 빌드 | 늘 |

스모크가 `?demo` 길을 봅니다(`src/smoke/smoke.spec.ts`). 스위치 상태는 시험하지 않습니다 —
배포마다 다른 값이라, 켜 둔 동안은 `/start` 를 열어 눈으로 확인합니다.
