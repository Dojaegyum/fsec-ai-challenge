...
> **2026-09-06 첫 실제 실행에서 어긋난 자리 둘** (심사위원 시점 QA 를 위해 켰습니다)
> - 스크립트는 `~/.ssh/id_ed25519_finally` 를 읽는데 위 손 순서의 키는 `~/.ssh/finally_oracle` 입니다.
>   같은 키를 그 이름으로 복사해 넘겼습니다 — 새로 만들 것 없이 `cp` 둘.
> - 필터 없이 만들면 **CUDA 12.4 호스트**에 떨어질 수 있고, 이미지(cu12.8.1)가 그 위에서 영영 안 뜹니다
>   (uptime 0 · ssh 안 열림 · 13분 낭비 · ②의 경고가 잡는 그 함정). `POD_SPEC` 에
>   `allowedCudaVersions: ["12.8","12.9","13.0"]` 을 박아 두 번째는 12.8 호스트로 바로 갔습니다.
>   ① → health ready 까지 **약 14분**, ⑤ 재배포 **약 5분**이 실측입니다.
