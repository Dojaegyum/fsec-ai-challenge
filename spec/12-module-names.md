...
이 문서를 작성한 시점에 `spec/`의 다른 문서들은 아직 「Ingest 서비스」·「2차 PII 스크러버」 같은 옛 표기를 쓰고 있습니다. **일괄 반영은 이 작업에 포함하지 않았습니다** — 같은 파일들을 다른 작업이 동시에 고치고 있었기 때문입니다.

도메인 용어는 하나도 바뀌지 않았으므로 반영 대상은 **모듈 이름뿐**입니다.

- TODO: 「서버 구성 요소」 표를 새 이름으로 ([08](08-api.md) §서버 구성 요소)
- TODO: 「2차 PII 스크러버」 → `pii-tokenizer` 표기 반영 ([04](04-pii-boundary.md) [08](08-api.md) [10](10-errors.md))
- TODO: 층 2 모듈 이름을 절 제목에 대응 ([11](11-chat-context.md) §2 §3 §4 §6 §8)
