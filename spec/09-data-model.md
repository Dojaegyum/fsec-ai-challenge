...
> `04-pii-boundary.md` 규칙 3의 24시간은 **볼트 항목의 수명**이고, 이 값은 **사건 자체의 수명**입니다. 서로 다른 것입니다. 규칙 3이 구 명칭 시점에 쓰였을 때는 둘이 같았지만, [ADR-0003](../decisions/0003-복원-매핑-보관-위치.md)으로 분리됐습니다.
...
`org_id`와 `org_name_raw`를 나눈 이유는 표기가 흔들리기 때문입니다. 사용자는 "국민", "KB국민은행", "국민은행"을 다 씁니다. `org_name_raw`에 들어온 그대로를 남기고, 정규화된 `org_id`로 KB를 찾습니다.
...
> ⬜ TODO(미정): 「막혔다」를 무엇으로 판정할지. 사용자가 물어보는 경우는 명확한데, 단계가 오래 미완료인 것을 자동 판정할지는 정하지 않았습니다.
