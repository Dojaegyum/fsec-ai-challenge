...
// 응답 200
{
  ...
  "reply_segments": [
    { "text": "지급정지를 거셨으면 다음은 피해구제 신청서 제출입니다.", "ref": "kb-2" },
    { "text": "8월 20일까지 하셔야 합니다.",                          "ref": "case-3" },
    { "text": "넘기면 은행이 9월 3일까지 추가 기간을 줍니다.",          "ref": "kb-2" },
...
**응답 본문이 문장 단위로 쪼개져 내려갑니다.** 각 문장이 어느 자료에서 나왔는지 `ref`가 가리킵니다 → [11](11-chat-context.md) §3.4 · §5.
...
**클라이언트는 `reply_segments`의 `text`를 순서대로 이어 붙여 표시합니다.** 부분 복원은 이어 붙인 뒤에 합니다 → [04](04-pii-boundary.md).
...
| 3 | `citations`의 각 항목이 적어도 한 문장에서 쓰였는가 | 과잉 인용 → 같음 |
