...
> 2026-08-16 [RFC-0002](../rfc/0002-복원-매핑-보관-위치.md) 채택으로 둘째·셋째가 바뀌었습니다.
...
**초과하면 `429`(`RATE_LIMITED`)입니다.** `Retry-After` 에 남은 창 시간이 들어갑니다 → [10-errors.md](10-errors.md) §3.1.
...
> ⬜ TODO(미정): 저장 위치. 사건당 카운터는 볼트 저장소(`KV_URL`)에 두는 것이 자연스럽지만 IP 카운터는 사건과 무관합니다.
...
**이 구조는 다른 엔드포인트에서도 나갑니다** (§3.9 되묻기, [10-errors.md](10-errors.md) §4). **정의는 이 절에만 두고 나머지는 참조합니다** — 모양이 갈라지면 프론트가 같은 것을 두 번 구현하게 됩니다.

| 칸 | 타입 | 필수 | 뜻 |
| --- | --- | :---: | --- |
| `slot_key` | string | Y | 채우려는 슬롯. [09](09-data-model.md) §5.1 목록에 있는 이름만 |
...
**`citations`가 이 답변이 무엇을 보고 쓰였는지 밝힙니다.** `ref`는 서버가 이번 턴 프롬프트에 발급한 번호입니다 → [11](11-chat-context.md) §3.4 · §5.
...
**이 경로에서는 재시도하지 않습니다.** 형식 오류가 아니라 근거가 없다는 선언이라 **같은 프롬프트로 다시 부르면 같은 답이 옵니다.** 재시도는 `ref` 검증을 어겼을 때만 합니다 → [10-errors.md](10-errors.md) §2.

**채울 슬롯이 더 없으면 그때 1332를 안내합니다** → [10-errors.md](10-errors.md) §4.1.
...
**「절차를 언급했는지」를 판정하지 않습니다.** 그 판정에 근거가 없기 때문입니다 → [11](11-chat-context.md) §6.1. 대신 **모든 문장이 `ref`로 출처를 밝히거나 `null`로 정보 없음을 밝힙니다.**

**서버가 확인하는 것은 넷입니다** → [11](11-chat-context.md) §6.2.

| # | 확인 | 어기면 |
| --- | --- | --- |
| 1 | `ref`가 이번 턴에 발급한 번호인가 | 지어낸 참조 → `KB_CITATION_MISSING`, 재시도 |
| 2 | `kb-` 항목의 `kb_entry_id`·`kb_version`이 발급한 값과 같은가 | 인용 바꿔치기 → 같음 |
| 3 | `citations`의 각 항목에 `why`가 채워져 있는가 | 형식 위반 → 같음 |
| 4 | `insufficient`가 `true`인가 | **되묻기.** 에러 아님 |

**1~3번은 형식 위반이라 재시도합니다. 4번은 재시도하지 않습니다.**

**3번은 형식 검사일 뿐입니다.** `why`의 내용이 실제 답변과 맞는지는 기계로 확인할 수 없습니다. **과잉 인용은 억제만 하고 검사로는 잡지 못합니다** — 이유를 쓰게 하면 관련 없는 항목은 쓸 말이 없어 스스로 걸러지지만, 그럴듯하게 지어 쓰면 통과합니다.
...
| 검사 | 무엇 | 정의된 곳 |
| --- | --- | --- |
| 인용 검증 | 절차를 언급했는데 `citations`가 비었는가 | [10](10-errors.md) §4 |
| 판단 근거 분리 | `reasoning`을 응답 본문에서 뺐는가 | 위 |
| 잔여 PII | 송출 직전 원문이 남았는가 | [04](04-pii-boundary.md) |
...
**`reply` 안의 토큰은 종류별로 부분 복원됩니다** → [04](04-pii-boundary.md) 「복원 위치와 범위」
