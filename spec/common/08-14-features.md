...
| **F-05b** | 슬롯 체커 | T0/T1/T2 티어링 — 증거 자동 추출 우선, 미충족 시 1문항([API](08-14-api.md) §3.4 `input` — 버튼으로 담을 수 있는 것은 전부 버튼), "모름" 시 보수적 슈퍼셋 플랜, 보완 시 플랜 자동 재생성 | ①③ | [02](../backend/08-14-slot-tiering.md) |
...
| ID | 기능 | 설명 | 요건 |
| --- | --- | --- | --- |
| **F-08** | 서류 기재 안내 | 피해구제신청서·자율배상 신청에 **무엇을 어느 칸에 적는지** 항목과 값을 짝지어 보여준다. 값 복원은 브라우저에서만. **`.docx` 생성은 P2** → [ADR-037](../../decisions/037-doc-guidance-not-generation.md) | ②③ |
