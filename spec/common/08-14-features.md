...
| **F-02** | 텍스트화 엔진 | STT(화자 분리)·OCR(대화 구조 보존) → 통합 전사 뷰 | ① | | **부분** — `transcriber` ↔ `services/transcriber`(`TRANSCRIBER_URL` · [ADR-052](../../decisions/052-stt-configuration.md)) · `transcript-viewer` 는 구현. **오디오 화자 분리는 구현 전** — STT 엔진이 `speaker` 를 안 채우고(`services/transcriber/engines/faster_whisper_stt.py` 끝의 ⬜) 앱이 `no_speakers` 로 실어 냅니다. 화자 A/B 가 붙는 것은 **이미지의 말풍선 좌·우**뿐(`src/modules/transcriber/transcribe.ts` `arrangeByColumn`). 주소가 없으면 전사만 멈추고 사건은 돕니다 (2026-09-07 손질 — 심사 발견 DC-04·E2-P1) |
...
| F-05b 슬롯 체커 | `slot-extractor`(2026-09-04 조립 · ADR-069) · `slot-checker` | `chat-handler` |
