...
| ID | 기능 | 설명 | 요건 | 상세 |
| --- | --- | --- | --- | --- |
| **F-01** | 사건 파일 생성·다중 업로드 | 녹음/이미지/텍스트 드래그&드롭 → 하나의 사건으로 묶음 | ⓪ | [60](60-screens.md) |
| **F-02** | 텍스트화 엔진 | STT(화자 분리)·OCR(대화 구조 보존) → 통합 전사 뷰 | ① | |
| **F-03** | PII 스크러버 | 클라이언트 정규식 1차 + 서버 NER 2차 토큰화. 복원 매핑은 클라이언트만 보관, 세션 파기 | ② | [40](40-pii-boundary.md) |
| **F-04** | 수법 판별·위험도 | 토큰화 텍스트 기반 LLM 분석 — 수법 분류·위험도·**근거 스팬 인용** | ① | |
| **F-05** | 경유 서비스 감지 + 매뉴얼 분기 | 이체내역·문진으로 경유 서비스 특정 → 8유형 KB(RAG) 기반 맞춤 플랜 생성 (근거·시행일 인용 강제) | ③ | [30](30-channel-matrix.md) |
| **F-05b** | 슬롯 체커 | T0/T1/T2 티어링 — 증거 자동 추출 우선, 미충족 시 버튼 1문항, "모름" 시 보수적 슈퍼셋 플랜, 보완 시 플랜 자동 재생성 | ①③ | [20](20-slot-tiering.md) |
| **F-06** | 실행 보드 | 체크리스트 + 골든타임 타이머 + 3영업일 D-day 트래커 + 진행 타임라인(환급 3~6개월 기대치 관리) | ③ | [60](60-screens.md) |
| **F-06b** | 완수 검증 엔진 | 부산물 입력/업로드로 완료 판정 (L1 자동검증 / L2 증빙 / L3 자기신고). 통화 동반 모드 포함 | ③ | [50](50-completion-hook.md) |
