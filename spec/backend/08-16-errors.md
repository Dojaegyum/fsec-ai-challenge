...
| `CASE_NOT_FOUND` | 404 | 해당 사건을 찾지 못했습니다. |
| `INTERNAL` | 500 | 처리 중 문제가 발생했습니다. |

> 2026-08-21 아래 셋이 추가됐습니다. 표가 도메인 실패만 담고 있어 「본문이 JSON 이 아니다」·
> 「관리자 인증이 없다」·「그런 사건이 없다」를 넣을 칸이 없었습니다.
...
**`CASE_NOT_FOUND` 는 `KB_ENTRY_NOT_FOUND` 와 다릅니다.** 저쪽은 절차 항목이 없는 것이고
이쪽은 사건이 없는 것입니다 — 사용자에게 보일 말이 완전히 다릅니다.
