...
> 예산 290초(`lib/llm.ts`)와 함수 상한 300초를 넘겨 503 이 납니다(2026-09-06 저녁 기준 · 그날 오후까지는 90초·100초, 그 전에는 45초·60초였습니다 — 우리가 먼저 끊지 않기로 하고 플랫폼 최대까지 열었습니다).
> 위 「3~8초」는 08-16 시점 추정이었습니다.
...
| `CASE_NOT_FOUND` | 404 | 이 주소의 사건을 찾을 수 없습니다. 마지막 활동일부터 180일이 지나면 자동으로 파기됩니다. |
...
| `KB_CHANGE_NOT_FOUND` | 404 | 그 변경을 찾지 못했습니다. (관리자 — [API](../common/08-14-api.md) §7.3 · 2026-09-06 ADR-088) |
| `KB_CHANGE_DECIDED` | 409 | 이미 판단이 끝난 변경입니다. (관리자 — 승인·거절은 다시 판단하지 않습니다 · ADR-044) |
...
> 2026-08-21 아래 둘이 추가됐습니다. 표가 도메인 실패만 담고 있어 「본문이 JSON 이 아니다」·
> 「관리자 인증이 없다」를 넣을 칸이 없었습니다.

**`UNAUTHORIZED` 의 문구는 일부러 아무것도 알려주지 않습니다.** 아이디가 틀렸는지 쿠키가
지났는지를 구분해 주면 그게 곧 힌트가 됩니다. 이 경로의 상대는 피해자가 아니라 운영자라
문구가 뭉뚱그려져도 피해자를 혼란시키지 않습니다.

**실패한 관리자 접근은 감사 표에 안 남깁니다.** 정본이 *"모든 에러 응답에 `audit_id` 가
붙습니다"*(§5)라고 했지만 **문지기는 예외입니다** — 인증 없는 요청이 감사 표에 줄을 쓰게 하면
그 자체가 밖에서 잠글 수 없는 쓰기 통로가 됩니다. 서버 로그에만 남습니다.

#### `CASE_NOT_FOUND` — 없는 것과 파기된 것을 가르지 않습니다

> 2026-08-21 신설 → [API 계약](../common/08-14-api.md) §3.10.

**「있었는데 지웠다」와 「없었다」에 같은 코드를 냅니다.** 가르면 **그 토큰이 한때
유효했다는 사실**이 밖으로 나가고, 익명 링크에서는 그것 자체가 정보입니다
([ADR-021](../../decisions/021-reentry-and-identity.md) 「토큰이 곧 인증」).

**대신 문구가 둘 다 설명합니다** — 위 표의 문구가 파기 안내를 함께 답니다.
API 를 나누지 않아도 사용자는 답을 얻습니다.

**`KB_ENTRY_NOT_FOUND` 와 다릅니다.** 저쪽은 절차 항목이 없는 것이고 이쪽은 사건이
없는 것입니다 — 사용자에게 보일 말이 완전히 다릅니다.

### 3.1 `Retry-After` — 503 과 429 에만 붙입니다
...
증거 업로드 상한(사건당 30개·300MB)은 창이 아니라 **사건이 사는 동안의 누적**이라 남은 창이라는
개념 자체가 없습니다 — 기다려도 풀리지 않습니다. 아무 숫자나 넣으면 사용자가 그 초마다 헛되이
다시 누릅니다. **헤더를 빼면 클라이언트가 「기다리면 된다」로 오해하지 않습니다.**
...
### 3.1.1 본문의 `retryable` — 브라우저가 버튼을 띄울지 정합니다

> 2026-08-23 확정. 제기 →
> [층 C 서버와 이야기하는 넷 계획](../../docs/plans/08-22-layer-c-transport.md) Task 1.

**브라우저에도 `retry-checker` 의 짝(`poll-checker`)이 있는데, 지금 본문으로는
같은 규칙을 지킬 수 없습니다.** HTTP 상태로 유추하면 뒤집힙니다 —
`IngestError` 는 §2 에서 **재시도 가능인데 422**, `RateLimitedError` 는
**재시도 불가인데 429** 입니다. 그래서 본문에 한 칸을 답니다.
...
```jsonc
{
  "error": {
    ...
    "audit_id": "01J8XKR2N4P6T8V0W2Y4A6C8E0",
    "retryable": false        // ← 더하는 칸
  }
}
```

#### ⚠️ §2 표의 값을 그대로 옮기지 마세요 — 다른 물음입니다

**두 자리가 같은 낱말을 쓰지만 묻는 것이 다릅니다.**

| | 묻는 것 | 언제 쓰이나 |
| --- | --- | --- |
| §2 의 `retryable` | **서버가 자기 안에서** 다시 시도할까 | 응답이 나가기 **전** |
| 본문의 `retryable` | **사용자가 같은 요청을 다시 보내면** 달라질까 | 응답이 나간 **뒤** |

**응답이 브라우저에 닿았을 때 서버의 재시도는 이미 끝났습니다.** §2 에서 ✓ 였다는 것은
「서버가 이미 해봤고 그래도 실패했다」는 뜻이지, 「사용자가 다시 누르면 된다」가 아닙니다.

**그대로 옮기면 뒤집히는 자리가 둘입니다.**

| 예외 | §2 | 본문 | 왜 |
| --- | :---: | :---: | --- |
| `KbCitationMissingError` (502) | ✓ | **`false`** | 「서버가 두 번 시도해 같은 결과였습니다. 기다린다고 달라지지 않습니다」 — 위 §3.1 표 |
| `IngestError` (422) | ✓ | **`false`** | 같은 파일은 같은 결과입니다. 문구도 「**다른 파일로** 시도해 주세요」입니다 |

#### 값

| HTTP | `retryable` | 왜 |
| --- | :---: | --- |
| **503** (`*_UNAVAILABLE`·`STORE_ERROR`) | `true` | 상대 서비스가 아픕니다. 시간이 지나면 나을 수 있습니다 |
| **429** (창 있음) | `true` | 남은 창이 지나면 풀립니다 |
| **429** (창 없음 — 증거 상한) | `false` | 사건이 사는 동안의 누적이라 기다려도 안 풀립니다 |
| **502** · **4xx 전부** | `false` | 요청이나 상태의 문제라 시간이 안 고칩니다 |

**지금은 `Retry-After` 유무와 값이 정확히 일치합니다.** 그래도 칸을 두는 것은
**판단에 이름을 붙여 두려는 것**입니다 — 브라우저가 헤더의 있고 없음으로 뜻을 **추론**하면,
나중에 둘이 갈릴 때 조용히 틀립니다.

#### 브라우저가 쓰는 법

**`retryable === true` 일 때만 「다시 시도」 버튼을 띄웁니다.**
`Retry-After` 가 함께 오면 「10초 뒤 다시 시도할 수 있습니다」처럼 시간을 같이 보입니다.

**여전히 자동으로 다시 부르지 않습니다** — 위 §3.1 그대로입니다. 이 칸은
**버튼을 띄울지**에만 쓰고, **누르는 것은 사용자가 합니다.**

**둘이 어긋난 응답(`retryable: true` 인데 `Retry-After` 없음)은 서버 결함입니다.**
그때 브라우저는 **`retryable` 을 따르고** 시간 없이 버튼만 띄웁니다 —
더 정확한 판단이 담긴 칸이기 때문입니다.
