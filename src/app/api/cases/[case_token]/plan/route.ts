  ...
  return handleRoute(request, async (ctx) => {
    ...
    return {
      body: {
        ...
        // 저장된 단계 중 **가장 최근에 만들어진** 시각 → `plan_step.generated_at`
        // (09-data-model.md §6). 재생성이 보존한 단계는 옛 시각을 그대로 들고
        // 있어(§6.1) 최대값이라야 「이 안내는 언제 기준인가」가 맞습니다.
        //
        // **지금 시각을 넣지 않습니다.** 조회는 아무것도 안 바꾸는데 화면이
        // 폴링할 때마다 「방금 갱신됨」이 됩니다 — 단계가 없으면 `null` 입니다
