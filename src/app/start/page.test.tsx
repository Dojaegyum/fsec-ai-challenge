 ...
 */
describe("예시 자료로 체험하기", () => {
  it("동작을 받으면 버튼이 그려진다", () => {
    expect(textOf(draw({ onMock: () => {} }))).toContain("예시 자료로 체험하기");
  });

  it("**동작을 안 받으면 안 그린다** — 눌러도 아무 일 없는 버튼을 만들지 않습니다", () => {
    expect(textOf(draw())).not.toContain("예시 자료로 체험하기");
  });

  /**
   * ⚠️ **2026-09-06 점검 — 「Mock」은 개발 용어입니다.** 이 칩은 실사용자에게
   * 안 보이는 것이 기본이지만(아래 `?demo`·스위치), 시연 화면은 심사위원이
   * 봅니다. 무엇을 하는 단추인지가 글자로 읽혀야 합니다
   */
  it("개발 용어를 화면에 쓰지 않는다", () => {
    expect(textOf(draw({ onMock: () => {} }))).not.toContain("Mock 파일로 실행");
  });
