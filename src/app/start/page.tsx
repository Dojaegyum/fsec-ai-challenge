 ...
 *
 * ## 1항도 다시 썼습니다 (2026-08-31) — 4항과 부딪히고 있었습니다
 *
 * 1항이 *「계좌·전화·이름은 전송 전 브라우저에서 토큰으로 치환되며」* 라고 적고
 * 있었는데 **둘 다 사실이 아니었습니다.**
 *
 * | 적혀 있던 것 | 실제 |
 * | --- | --- |
 * | 이름도 브라우저에서 가린다 | 1차는 정규식이라 **한국 이름을 못 잡습니다**. 가리는 것은 서버 2차(`pii-tokenizer`)입니다 |
 * | 전송 전에 다 가린다 | 올리신 녹음·이미지는 **읽어야 가릴 수 있어** 원본이 전사 서버를 지납니다 — 4항이 그렇게 적고 있습니다 |
 *
 * 같은 모달 안에서 1항과 4항이 다른 말을 했습니다. 동의를 받는 글이라
 * **읽는 사람이 어느 쪽을 믿을지 알 수 없는 상태**였습니다.
 *
 * ## 아직 안 붙은 것
 *
 ...
 *
 * ## ✅ 전사·판독 서버의 소재지를 확인했습니다 (2026-08-31)
 *
 * 그전까지 「국내 원칙 · 국외일 수 있음」이었습니다. `TRANSCRIBER_URL` 이 배포
 * 환경변수에만 있어 코드로 확인할 수 없었기 때문입니다(ADR-059).
 *
 * 그 서버에 직접 물어봤습니다 — OCI 인스턴스 메타데이터가 답한 값입니다.
 *
 * ```
 * region                us-ashburn-1
 * availabilityDomain    BNJD:US-ASHBURN-AD-1
 * shape                 VM.Standard.A1.Flex
 * ```
 *
 * **미국 버지니아입니다.** 「국외일 수 있음」이 아니라 **국외입니다** —
 * 원본 음성·이미지가 실제로 국경을 넘고 있고, 그것이 이 표에서 가장 무거운 줄입니다.
 * ⚠️ **주소가 바뀌면 이 줄도 함께 바꾸세요.** 사실이 바뀌었는데 글이 안 바뀌면
 * 그게 우리가 고친 그 거짓말로 돌아가는 자리입니다.
 *
 * ⬜ **운영 전에 국내로 옮겨야 합니다** → ADR-043 이 「운영은 국내에서만」이라고
 * 정했습니다. 지금은 무료 등급이라 자리를 못 잡았고, **발표에서 그대로 밝힙니다.**
 *
...
const 위탁 = [
  ...
  [
    "전사·판독 서버",
    // **확인한 값입니다** — 위 「소재지를 확인했습니다」. 「일 수 있음」이 아닙니다
    "국외 (미국 버지니아)",
  ...
  return (
    <>
      <Clause title="1. 수집하는 항목" checked={checks[0] ?? false} onToggle={() => onToggle(0)}>
        ...
        {/* ⚠️ **가리는 자리가 하나가 아닙니다.** 2026-08-31 까지 이 조항이
            *「계좌·전화·이름은 전송 전 브라우저에서 토큰으로 치환되며」* 였는데,
            브라우저 1차는 정규식이라 **이름을 못 잡습니다**(한국 이름에 쓸 수 있는
            규칙이 없습니다). 이름을 가리는 것은 서버 2차이고, 그 사실이
            4항의 위탁 표(원문이 전사 서버를 지납니다)와 정면으로 어긋났습니다 */}
        <p className="mt-2 text-[12.5px] leading-[1.6] text-ink-3">
          숫자로 된 것(계좌·카드·전화·주민등록번호)은{" "}
          <b className="font-[620] text-ink-2">이 브라우저에서, 보내기 전에</b> 토큰으로
          바뀝니다. <b className="font-[620] text-ink-2">이름처럼 모양으로 못 가리는 것</b>은
          저희 서버가 받아 <b className="font-[620] text-ink-2">외부 AI 로 내보내기 전에</b>{" "}
          가립니다 — 즉 이름은 저희 서버까지는 원문으로 옵니다. 올리신 녹음·이미지도
          읽어야 가릴 수 있어 <b className="font-[620] text-ink-2">전사·판독까지는 원문</b>이
          지납니다 (4항).
        </p>
      </Clause>
      ...
      >
        ...
        <p className="mt-2.5">
          ...
          <b className="font-[620] text-ink-2">이 문서의 공개로 고지를 갈음합니다.</b>
        </p>
        {/* ⚠️ **원칙과 지금을 나눠 적습니다.** 2026-08-31 까지 이 자리가
            *「전사·판독은 국내에서만 처리하는 것을 원칙으로 하고」* 만 적었는데,
            **그 서버는 지금 미국에 있습니다**(위 `위탁` 의 소재지 확인). 원칙만
            적고 지금을 안 적으면 읽는 사람은 국내로 읽습니다 */}
        <p className="mt-2.5">
          원문이 지나는 전사·판독은{" "}
          <b className="font-[620] text-ink-2">국내에서만 처리하는 것을 원칙</b>으로 합니다.
          다만 <b className="font-[620] text-deadline-urgent">지금 이 서비스의 전사·판독
          서버는 미국에 있습니다</b> — 시험 단계라 무료 등급 서버를 쓰고 있고, 정식
          운영 전에 국내로 옮깁니다. 성능 시험용으로 빌리는 국외 GPU 에는{" "}
