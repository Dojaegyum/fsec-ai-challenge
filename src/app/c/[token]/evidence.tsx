        ...
        {status === "processing" ? (
          ...
          <div className="grid gap-3 p-[18px_16px]">
            ...
            <div className="flex flex-wrap items-center gap-2">
              {/* **막지 않고 갈림길을 줍니다** — 그러려면 갈림길이 실제로 있어야 합니다.
                  2026-08-27 까지 이 버튼 둘에 `onClick` 이 없어 문구만 남아 있었습니다 */}
              <button
                type="button"
                onClick={() => pick.current?.click()}
                disabled={token === null || uploads.busy}
              ...
              {/* 「없이 진행」을 버튼으로 두지 않습니다 — 누를 것이 없습니다.
                  사건은 이미 진행 중이고 이 화면이 막고 있지 않습니다 */}
              <span className="text-[13px] text-ink-3">그냥 두셔도 됩니다.</span>
            </div>
          </div>
        ) : status === "pending" ? (
