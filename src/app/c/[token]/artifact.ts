 ...
 * | 접수번호를 적음 | L1 | 형식이 맞으면 `done_verified`, 아니면 `in_progress` |
 * | 했다고만 말함 | L3 | `unconfirmed` — **완료가 아닙니다** |
 *
 * ⬜ **지금 L1 은 언제나 실패합니다.** 기관별 접수번호 형식의 정본이 없어
 * `format_unknown` 으로 떨어집니다 → `completion-checker/README.md`.
 * **그래도 막히지 않습니다** — 서버가 `next_options` 로 L2·L3 를 함께 냅니다.
 *
 * ## 실패해도 사용자를 막지 않습니다
 *
