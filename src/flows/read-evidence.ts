...
export async function collectReading(
  ...
  const later = async () => {
    ...
    // **추출기는 진술 쪽과 한 몸입니다** → `flows/extract-slots.ts` · ADR-087
    await extractSlotsFrom(
      {
        caseId: input.caseId,
        sourceRef: input.evidenceId,
        // 자료 종류를 한 줄 앞세웁니다 — 「받는 계좌」가 상대 계좌라는 것은 이체 내역 캡처일 때의 뜻입니다
        sourceLabel: `자료 종류: ${KIND_LABEL[input.kind]}`,
        text: masked.lines
          .map((one) => one.text)
...
async function repairOrgs(
  ...
  try {
    ...
    // 여기서 던지면 **미룬 일의 나머지가 통째로 안 돕니다.** 전사 결과는 이미
    // 저장돼 있습니다 — 이 함수는 `finish` 뒤 `after()` 안에서 도니까요(ADR-086).
    // 잃는 것은 뒤이은 슬롯 추출(`extractSlotsFrom`)이고, 그러면 금액·시각의
    // 되묻기가 통째로 안 생깁니다. 응답도 이미 나간 뒤라 아무 데도 안 남습니다
  }
}

/**
 * 자료 종류를 프롬프트 첫 줄에 앞세웁니다 → `flows/extract-slots.ts`.
 *
 * 「받는 계좌」가 상대 계좌라는 것은 **이체 내역 캡처일 때의 뜻**입니다 — 종류를 안 알려 주면
 * 통화 녹음에서도 같은 규칙으로 읽습니다. 진술 쪽은 이 자리에 「사용자 진술」이 옵니다(ADR-087).
 */
