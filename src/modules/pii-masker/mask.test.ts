...
describe("계좌", () => {
  ...
  it("13자리가 주민번호 형태와 겹치면 주민번호로 본다", () => {
    ...
    // 계좌를 주민번호로 보면 계좌 슬롯이 안 채워질 뿐이지만, 주민번호를 계좌로 보면
    // 계좌 이름표를 단 주민번호가 계좌 자리(슬롯·서류)에 되살아납니다 → 04 「1차가 잡는 것」.
  ...
  it("공백을 허용해도 날짜·전화는 건드리지 않는다", () => {
    ...
    // 전화가 먼저 가져갑니다 — 이름표가 바뀌면 종류별 건수(§1.1)와 슬롯 자리가 달라집니다
...
/**
 * 이름은 1차가 **찾지** 않습니다 — 정규식으로 한국 이름을 잡으면 오탐이 폭발합니다.
 * 그런데 서버 2차가 붙인 `[이름-1]` 의 짝은 응답으로 받아 볼트에 있고(ADR-062 · ADR-075),
 * 서버 장부에는 원문이 없어 같은 이름을 다시 말하면 서버가 `[이름-2]` 를 새로 붙였습니다.
 * **이미 아는 이름은 보내기 전에 브라우저가 그 이름표로 바꿉니다** → ADR-079.
 * 「같은 값 → 같은 번호」가 이름에도 서게 됩니다.
 */
describe("이미 아는 이름은 그 이름표로 바꾼다 — 찾지는 않는다 (ADR-079)", () => {
  const KNOWN = [{ token: "[이름-1]", kind: "이름" as const, seq: 1, original: "김민수" }];

  it("문맥에 있는 이름 원문이 나오면 같은 이름표로 바꾼다 — 몇 번이든", () => {
    const r = maskText("김민수입니다. 김민수 맞아요", { mappings: KNOWN });
    expect(r.masked).toBe("[이름-1]입니다. [이름-1] 맞아요");
    // 새로 만든 것은 없습니다 — 봉해 맡길 것도 없습니다
    expect(r.added).toEqual([]);
  });

  it("모르는 이름은 건드리지 않는다 — 그건 서버 2차의 몫이다", () => {
    const r = maskText("박철수입니다", { mappings: KNOWN });
    expect(r.masked).toBe("박철수입니다");
    expect(r.added).toEqual([]);
  });

  it("긴 이름이 짧은 이름을 품으면 긴 쪽이 이긴다", () => {
    const both = [
      { token: "[이름-1]", kind: "이름" as const, seq: 1, original: "김민" },
      { token: "[이름-2]", kind: "이름" as const, seq: 2, original: "김민수" },
    ];
    const r = maskText("김민수와 김민이 왔어요", { mappings: both });
    expect(r.masked).toBe("[이름-2]와 [이름-1]이 왔어요");
  });

  it("계좌와 섞여도 각자 자리를 지킨다", () => {
    const r = maskText("김민수 110-123-456789 로 보냈어요", { mappings: KNOWN });
    expect(r.masked).toBe("[이름-1] [계좌-1] 로 보냈어요");
    expect(r.added.map((m) => m.token)).toEqual(["[계좌-1]"]);
  });

  it("원문이 없는 예약 칸은 쓰지 않는다 — 대역 문자열을 찾을 일이 없다", () => {
    const reserved = [
      { token: "[이름-3]", kind: "이름" as const, seq: 3, original: "\u0000볼트에-있으나-이-기기에서-못-여는-자리" },
    ];
    const r = maskText("김민수입니다", { mappings: reserved });
    expect(r.masked).toBe("김민수입니다");
  });
});

/**
 * 한 글자 조각은 개인정보 원문이 아닙니다 → ADR-081.
 *
 * 2026-09-06 배포본 QA 에서 서버가 「요」(이름)·「8」(주민번호)·「2026」(계좌)을 원문으로 하는
 * 대응표를 만들어 보냈고, 브라우저는 그것을 볼트에 넣고 ADR-079 대로 재사용했습니다. 그 뒤
 * 그 기기에서는 「요」가 든 발화가 전부 가려 나갔고, 「8」·「2026」이 든 글은 누출 검산에 걸려
 * **네트워크 호출 없이 영구 차단**됐습니다. 원인은 서버에 있지만(그쪽은 ADR-081 앞부분이 막습니다)
 * 브라우저도 짧은 조각을 믿지 않습니다.
 */
describe("원문의 최소 길이 (ADR-081)", () => {
  it("한 글자 이름 대응표는 문맥에 넣지 않는다 — 「요」", () => {
    const ctx = {
      mappings: [{ token: "[이름-5]", kind: "이름" as const, seq: 5, original: "요" }],
    };
    expect(maskText("지급정지는 오늘 오전에 했어요", ctx).masked).toBe(
      "지급정지는 오늘 오전에 했어요",
    );
  });

  it("두 글자 이름은 그대로 바꾼다 — 하한이지 금지가 아니다", () => {
    const ctx = {
      mappings: [{ token: "[이름-2]", kind: "이름" as const, seq: 2, original: "민수" }],
    };
    expect(maskText("민수라고 했어요", ctx).masked).toBe("[이름-2]라고 했어요");
  });

  it("누출 검산은 4자 미만 원문을 대조하지 않는다 — 「8」 · 「202」", () => {
    const short = [
      { token: "[주민번호-1]", kind: "주민번호" as const, seq: 1, original: "8" },
      { token: "[계좌-3]", kind: "계좌" as const, seq: 3, original: "202" },
    ];
    expect(() =>
      assertNoLeak("8월에도 비슷한 전화가 왔습니다 2026년", short),
    ).not.toThrow();
  });

  it("누출 검산은 4자 이상 원문이 남아 있으면 여전히 던진다", () => {
    const real = [
      { token: "[계좌-1]", kind: "계좌" as const, seq: 1, original: "110-2345-678901" },
    ];
    expect(() => assertNoLeak("국민은행 110-2345-678901 로 보냈어요", real)).toThrow(
      /원문이 남아 있습니다/,
    );
  });
});
