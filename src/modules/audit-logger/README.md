...
| 무엇 | 어떻게 | 왜 |
| --- | --- | --- |
| **상세 직렬화** | **키를 정렬한다** | 저장소가 JSONB 라 읽어올 때 키 순서가 달라지면, 같은 내용인데도 해시가 어긋나 멀쩡한 사슬이 깨진 것처럼 보입니다 |
| **토큰 검사** | **거부하고 아무것도 쌓지 않는다** | 볼트가 살아 있는 동안 토큰으로 원문을 얻을 수 있습니다 |
| **저장소 인터페이스** | `append` 와 `lastHash` 만 | 수정·삭제 함수가 없어 저장소를 어떻게 구현하든 그 경로가 생기지 않습니다 |
