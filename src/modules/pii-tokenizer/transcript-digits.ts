 ...
 *
 * ## 다시 쟀습니다 (docs/research/15 · large-v3 까지)
 *
 * 모델을 올리면 **「자리표기 변형」이 2 -> 4건으로 늘어납니다** — `medium` 이 아예
 * 못 듣던 계좌번호를 `large-v3` 는 듣고 날짜로 적기 때문입니다. **그 늘어난 몫까지
 * 이 규칙이 잡습니다.**
 *
 * | 전사 | 누출 | 과차단 |
 * | --- | --- | --- |
 * | medium | 1건 (`gapJoin` 5 에서는 2건) | 0건 |
 * | large-v3-turbo | **0건** | 0건 |
 * | large-v3 | **0건** | 0건 |
 *
 * `medium` 에 남은 1건은 `5501234567` 이 `"501-34567"` 로 읽혀 **8자리만 남은** 것이라
 * `minDigits` 아래로 떨어진 경우입니다. `large-v3` 에서는 사라집니다 —
 * **전사 품질이 이 규칙의 누출을 줄입니다.**
 *
 * 회귀는 `transcript-digits.leak.test.ts` 가 지킵니다.
 *
 ...
 * `minDigits` 는 8도 10도 같은 결과였습니다 — 여유가 있다는 뜻이지 이 값이 옳다는
 * 뜻은 아닙니다. 실제 통화로 다시 재야 합니다.
 */
...
/**
 * ⚠️ **`gapJoin` 은 5 였다가 8 로 넓혔습니다** -> docs/research/15-STT-GPU-실측.md §6.
 *
 * 5 로는 「마이너스」를 못 건넙니다. 전사가 하이픈을 그렇게 읽는 일이 있습니다.
 *
 * ```
 * 7777-01-9988776  ->  "7777년 1월 마이너스 9988776"
 *                              +-- "월 마이너스 " = 7자 > 5 -> 안전이 안 거둬짐
 *                                  -> 9988776(7자리)이 홀로 남고 minDigits 미만이라 안 가려짐
 * ```
 *
 * 계좌번호 뒷자리 7개가 그대로 나갔습니다. 8 로 넓히면 잡히고 **과차단은 그대로
 * 0 건**입니다(전사 조건 넷 x 40발화). 10·12 도 결과가 같아 8 이 하한입니다.
 */
export const DEFAULT_DIGIT_RULE: DigitRule = { minDigits: 9, gapJoin: 8 }
