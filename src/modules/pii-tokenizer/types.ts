...
export interface TokenizeContext {
  ...
  /**
   * **이 글이 전사문인가** → `transcript-digits.ts`.
   *
   * 켜면 숫자에 대해 **기본값이 뒤집힙니다** — 안전한 형태로 확인되지 않은
   * 숫자 덩어리를 가립니다.
   *
   * 왜 갈라야 하나: 실측이 *"깨끗한 텍스트에서 숫자 20건을 전부 잡던 정규식이
   * 전사문에서는 6~10건만 잡는다"* 를 보여줬습니다(docs/research/09 §5.4).
   * 값이 사라져서가 아니라 **숫자는 다 남고 구분자만 깨지기** 때문입니다 —
   * `110-234-567890` 이 `"110에 234-56만 7,890"` 으로 전사됩니다.
   *
   * **타이핑한 글에 켜지 마세요.** 거기서는 정규식이 제대로 잡고, 이 규칙을
   * 얹으면 멀쩡한 숫자까지 가려집니다.
   */
