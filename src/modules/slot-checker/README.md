...
| 무엇 | 어떻게 | 왜 |
| --- | --- | --- |
| **`extracted` 상태** | **채워진 것으로 센다** | 흐름이 「자동 추출 → T1 충족?」 순서라, 뽑힌 값이 있으면 플랜을 만들 수 있습니다 |
| **`unknown` 상태** | **값은 아니지만 다시 묻지 않는다** | 사용자가 이미 모른다고 답했으니 또 물으면 막아 세우는 것이 됩니다. 대신 슈퍼셋 플랜으로 갑니다 |
| **「모름」 선택지** | **없으면 붙여 준다** | 문구를 주는 쪽이 빠뜨려도 스펙 위반이 일어나지 않게 합니다 |
| **T2 질문 순서** | `org_name` → `amount` → `occurred_at` | ⬜ 정본에 없습니다. 조회 범위를 가장 크게 좁히는 것부터 두었습니다 |

## 증거에서 뽑힌 값은 되묻습니다 (2026-09-04 · ADR-069)

`extracted` 의 정의는 처음부터 「묻는 것은 『이 값이 맞나요』」였는데, 되묻는 코드는 `org_name` 의
사전 선택지뿐이었습니다. 이제 `CONFIRMABLE_KEYS` 다섯(금액·시각·상대 계좌·사칭 기관·연락 수단)이
`extracted` 이고 값이 있으면 **문진 순서 그 자리에서** 버튼으로 되묻습니다 —
`[맞아요] [아니에요, 다시 적을게요] [모름·기억 안 남]`. 문구는 `QuestionSource.confirmFor` 가 냅니다
(`src/lib/questions.ts`). 그 문구가 없으면(구현하지 않은 문구 소스) 되묻지 않고 지금까지처럼 채워진 것으로만 셉니다.

앞 두 선택지의 글자는 `CONFIRM_YES`·`CONFIRM_NO` 상수이고 `flows/answer-slot.ts` 가 같은 글자로 가릅니다 —
문구를 고치려면 상수를 고치세요. 되묻기가 T1 을 새치기하지 않습니다 — `transferred`·`channel` 이 비어 있으면 그쪽이 먼저입니다.
