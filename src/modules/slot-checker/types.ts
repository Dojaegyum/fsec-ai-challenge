...
export type SlotKey =
  ...
  | 'notice_started_at'
  /**
   * 피해자 본인 계좌(돈이 나간 쪽)의 **토큰**. 묻지 않고 자료에서만 뽑습니다 — 기재 안내의
   * 「내 계좌」·「환급 계좌」 제안 칸에 갑니다 → ADR-070. 절차 선택에는 안 쓰여 T2 충족에 안 셉니다
   */
  | 'victim_account'
  /**
   * 피해자 본인 이름의 **토큰**(`[이름-1]`). 이름 토큰은 2차 탐지가 켜졌을 때만 생기므로
   * 꺼져 있으면 이 슬롯은 비어 있습니다 — 원문 이름은 서버에 두지 않습니다 → ADR-070
   */
  | 'victim_name'
