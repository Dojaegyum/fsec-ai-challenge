...
export interface WaitCardProps {
  ...
  deadline: Deadline;
  /**
   * 공고가 시작된 시점 (ISO). §3.7 의 `starts_at`.
   *
   * **서버가 보냅니다** (§3.7 `starts_at`). 안 오면 이 카드를 그리지 않습니다 —
   * 화면이 대신 만들면 기기 시계를 믿게 됩니다.
   */
  startAt: string;
  /**
   * 시작~만료 사이에서 지금이 어디인가. `0`~`1`. §3.7 의 `elapsed`.
   *
   * **화면이 만들 수 없습니다** — 만들려면 기기 시계를 읽어야 하고, 날짜가
   * 틀린 사용자에게 「공고가 끝났다」를 잘못 보여줍니다. 기준 시계는 서버입니다
   * → spec/common/08-16-deadline-rules.md.
   */
  progress: number;
 ...
 *
 * 진행 막대의 `--horizon` 은 **「제도가 흐르는 시간」**을 뜻합니다 →
 * [ADR-048](../../../decisions/048-horizon-carries-meaning.md). 앰버를 피하려고 고른 색입니다.
 * 그 결정이 조건 둘을 달았으니 **지키세요** — 막대·점은 전부 `aria-hidden` 이고,
 * **뜻은 아래 글자 셋이 싣습니다**(「지금」은 색이 없어도 「지금」입니다).
 */
