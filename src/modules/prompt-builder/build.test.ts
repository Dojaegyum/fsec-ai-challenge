...
describe('시스템 지시문에 금지 사항이 하나도 빠지지 않았다', () => {
  ...
  it('**방금 한 말을 먼저 받아 주라는 지시가 있다** — ADR-076', () => {
    // 배포본에서 첫 진술에 답이 진술을 한 마디도 받지 않고 「지금 바로 112에」로
    // 시작했습니다. 사용자는 자기 말이 읽혔는지 알 수 없었습니다
    expect(SYSTEM_PROMPT).toContain('방금 한 말을 받아 준다')
    // 이력에는 참조 번호가 없어 인용할 수 없습니다 — 지어낸 ref 로 검증에 걸리면 안 됩니다
    expect(SYSTEM_PROMPT).toContain('citations 에 적지 않는다')
  })

  it('**추정 기한을 확정처럼 말하지 말라는 지시가 있다** — 기한 규칙', () => {
    // 화면에는 「미확인」 배지가 붙는데 챗만 단정하면 두 자리가 다른 말을
    // 하고, 사용자는 더 단정적인 쪽을 믿습니다. 실제로 이 줄이 없을 때
    // 모델이 날짜만 옮기고 「확정이 아닙니다」를 흘리는 것을 확인했습니다
    expect(SYSTEM_PROMPT).toContain('아직 확정이 아니라고 적혀 있으면')
    expect(SYSTEM_PROMPT).toContain('날짜만 옮기고 그 말을 빠뜨리면 안 된다')
  })

  it('**질문에는 그 답이 먼저 온다는 지시가 있다** — 답변 구조 개정 (2026-09-06)', () => {
    // 배포본에서 "3영업일이 무슨 뜻이에요" 같은 물음에도 다음 할 일부터
    // 말해 질문이 묻혔습니다
    expect(SYSTEM_PROMPT).toContain('사용자가 무엇을 물었으면 그 답이 첫 문장이다')
  })

  it('**직전 턴에 이미 안내한 다음 할 일은 한 문장 이내로만 되짚으라는 지시가 있다** — 답변 구조 개정 (2026-09-06)', () => {
    // 매 턴 같은 전화 시간·접수번호 당부가 되풀이돼 답이 지루해졌습니다
    expect(SYSTEM_PROMPT).toContain('한 문장 이내로만 되짚는다')
    expect(SYSTEM_PROMPT).toContain('전화 시간·접수번호 당부를 다시 적지 마라')
  })
})
