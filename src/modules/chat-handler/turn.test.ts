...
describe("근거는 매뉴얼 항목만 밝힌다", () => {
  ...
  /**
   * ⚠️ **화면이 「…을 보고 안내했습니다」로 이어 붙이고 있었습니다** (2026-09-06 점검).
   * 제목이 「112에 신고합니다」처럼 받침 없이 끝나면 **「합니다을」** 이 됩니다 —
   * 조사는 앞 글자에 따라 달라지는데 매뉴얼 제목은 KB 가 정합니다.
   * 그래서 **제목을 따옴표로 감싸** 조사가 제목에 붙지 않게 합니다.
   */
  it("제목을 「」 로 감싼다 — 뒤에 오는 조사가 제목에 붙지 않게", () => {
    expect(sourceNote([{ ref: "kb-1", label: "112에 신고합니다" }])).toBe("「112에 신고합니다」");
  });

  it("여럿이면 **각각** 감싸고 가운뎃점으로 잇는다", () => {
    expect(
      sourceNote([
        { ref: "kb-1", label: "112에 신고합니다" },
        { ref: "kb-2", label: "피해구제 신청서 제출" },
      ]),
    ).toBe("「112에 신고합니다」 · 「피해구제 신청서 제출」");
  });
