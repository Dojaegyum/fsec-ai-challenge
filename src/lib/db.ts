...
/** `SlotWriter.write` 가 받는 것. 사건을 열 때 함께 넣는 슬롯도 같은 모양입니다 → ADR-076 */
export type SlotWriteInput = Parameters<SlotWriter['write']>[0]
 ...
 *
 * **`sql` 을 인자로 받는 이유**는 트랜잭션 안에서도 불러야 하기 때문입니다 —
 * `db-plan.ts` 의 `openCase` 가 시작 화면의 답을 사건·플랜과 **한 트랜잭션**에
 * 넣습니다(ADR-076 · ADR-046). 문장이 두 벌이면 한쪽만 고쳐지는 날이 옵니다.
 */
export async function upsertSlot(sql: Sql, input: SlotWriteInput): Promise<void> {
  await sql`
...
export function createSlotWriter(sql: Sql): SlotWriter {
  return {
    write: (input) => upsertSlot(sql, input),
  }
}
