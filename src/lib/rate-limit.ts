 ...
 * ## 저장 위치 — 아직 미정입니다
 *
 * ⬜ 정본 §1.3 의 마지막 줄이 *"TODO(미정): 저장 위치"* 입니다. 사건당 카운터는
 * 볼트 저장소(`KV_URL`)가 자연스럽지만 IP 카운터는 사건과 무관합니다.
 *
 * **그렇다고 「부르면 터지는 대역」으로 두지 않았습니다.** 다른 미설정 자원과
 * 다릅니다 → [not-configured.ts](./not-configured.ts). 속도 제한은 **모든 요청이
 * 지나는 길목**이라, 터지게 두면 사건 생성도 조회도 전부 500 이 됩니다.
 * 「제한이 정상 사용을 막으면 안 된다」의 가장 심한 형태입니다.
 *
 * 그래서 **프로세스 메모리에 세는 것**을 기본으로 둡니다. 그 대신 그게 무엇을
 * 못 하는지를 숨기지 않습니다 — `storeKind` 가 `'memory'` 로 드러나고 설정
 * 현황에 한 줄로 나옵니다 → [config-report.ts](./config-report.ts).
 *
 * | | 프로세스 메모리 | 공유 저장소 |
 * | --- | --- | --- |
 * | 인스턴스 하나 | 정확 | 정확 |
 ...
 *
 * 저장소가 정해지면 [`RateCounterStore`](#RateCounterStore) 하나만 갈아 끼웁니다.
