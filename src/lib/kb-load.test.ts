...
describe('⚠️ 유형 파일도 함께 실린다', () => {
  ...
  /**
   * **모델이 보는 KB 본문은 `summary` 한 칸입니다** — `lib/adapters.ts` 의
   * `kbRowToPromptEntry` 가 그것만 옮깁니다. `deadline` 은 날짜 계산기가 읽는
   * 구조라 모델에게는 안 갑니다.
   *
   * 그래서 사용자 기한을 `deadline` 에만 적고 문장에 안 쓰면, 그 기한이 아직
   * 계산되지 않은 사건(신청 전)에서 **모델은 기한이 있다는 것 자체를 모릅니다.**
   * 2026-09-06 배포본에서 *"3영업일이 정확히 무슨 뜻이에요? 주말도 포함되나요?"*
   * 에 모델이 「자료에 영업일 정의가 없다」며 `insufficient` 를 냈고, 사용자는
   * 답 대신 기관을 묻는 되묻기를 받았습니다 — 심사위원이 기한 지식을 시험하는
   * 바로 그 자리입니다 (rfc/002 「적재 전 자기점검」).
   */
  describe('사용자 기한은 모델이 보는 문장(summary)에도 말로 있다', () => {
    ...
    const userDeadlines = plan.rows.filter((r) => {
      const deadline = r.body.deadline as { owner?: unknown } | null
      return deadline?.owner === 'user'
    })

    it('사용자 기한이 있는 항목이 실제로 있다 — 없으면 아래가 헛돈다', () => {
      expect(userDeadlines.map((r) => r.kb_entry_id)).toContain('common-relief-documents')
    })

    it.each(userDeadlines.map((r) => [r.kb_entry_id, r] as const))(
      '%s 의 summary 가 기한을 숫자와 단위로 말한다',
      (_id, row) => {
        const deadline = row.body.deadline as { kind: string; amount: number }
        const summary = row.body.summary as string
        expect(summary).toContain(`${deadline.amount}${UNIT_WORD[deadline.kind] ?? ''}`)
      },
    )

    it('**영업일로 정한 기한은 무엇을 빼고 세는지도 말한다** — 「주말도 포함되나요」의 근거', () => {
      // 영업일에서 빼는 날 = 주말 + 공휴일 (+ 은행은 근로자의 날) → research/01 §2.1 · research/05
      for (const row of userDeadlines) {
        const deadline = row.body.deadline as { kind: string }
        if (deadline.kind !== 'business_days') continue
        const summary = row.body.summary as string
        expect(summary, row.kb_entry_id).toMatch(/토요일|주말/)
        expect(summary, row.kb_entry_id).toMatch(/공휴일/)
      }
    })
  })
