...
export function configReport(container: Container): readonly PortStatus[] {
  ...
  return [
    ...
    // 모델은 정해졌습니다(gemma3:4b · research/09 R-1). 비어 있다면 이유는
    // 「미선정」이 아니라 **주소가 없다**입니다 → ner.ts · ARCHITECTURE §10
    row('개인정보 2차 탐지', ports.ner !== null, ['NER_URL'],
    ...
    row('녹음 전사', !isUnconfigured(ports.stt), ['TRANSCRIBER_URL'],
