 ...
 *
 * **2026-08-27 에 그 차이를 다시 쟀습니다** — 결제한 그록으로 옮긴 뒤에도
 * 이 컴퓨터 4.1~6.9초 · 배포 환경 13.6~30.9초입니다. 제공자를 바꿔도 남는
 * 차이라 **여기(icn1)에서 나가는 길**이 그만큼 느린 것으로 봅니다.
 */
 ...
 *
 * ⚠️ **20초로 뒀다가 되돌렸습니다** (2026-08-27, 같은 날). 그 값은 무료
 * 한도의 `gemini-3.6-flash`(3~7초)에 맞춘 것이었는데, 결제한 그록으로 옮기니
 * 배포 환경에서 **성공한 응답이 13.6 · 16.7초**였습니다. 상한이 멀쩡한 답을
 * 자르고 다시 보내는 것이 로그에 그대로 찍혔습니다.
 *
 *     [llm] grok-4.5 · 시간 초과 · 20002ms      ← 자름
 *     [llm] grok-4.5 · HTTP 200  · 13612ms      ← 다시 보내서 성공
 *
 * 35초는 **관측된 최악(30.9초)보다 위**이고, 55초 예산에서 **두 번째 시도에
 * 20초를 남깁니다.** 한 후보가 예산을 다 먹는 것은 여전히 막습니다 —
 * 그게 이 상한이 있는 이유입니다.
 */
