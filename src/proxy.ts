 ...
 *
 * ⬜ **`audit_id` 가 없습니다.** 정본이 *"모든 에러 응답에 `audit_id` 가 붙습니다"*
 * 라고 했는데(08-16-errors.md §5) 문지기는 감사 기록을 남길 수 없습니다 —
 * 저장소에 붙지 않고(Next 문서가 공유 상태를 쓰지 말라고 합니다), **인증 없는
 * 요청이 감사 표에 줄을 쓰게 하면 그 자체가 밖에서 여는 수도꼭지**가 됩니다.
 * 실패한 관리자 접근을 어디에 남길지는 정해야 하는 자리입니다.
 */
 ...
 * 크론 호출인가.
 *
 * ⬜ **검증 방식이 정본에 없습니다** → ADR-025 「남은 것」의
